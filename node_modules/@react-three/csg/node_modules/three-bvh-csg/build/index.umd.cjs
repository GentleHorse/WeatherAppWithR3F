(function (global, factory) {
	...
	class Brush extends three.Mesh {
		...
		prepareGeometry() {
			...
			if ( ! geometry.boundsTree ) {
				...
				geometry.boundsTree = new threeMeshBvh.MeshBVH( geometry, { maxLeafTris: 3 } );
	...
	function collectIntersectingTriangles( a, b ) {
		...
		a.geometry.boundsTree.bvhcast( b.geometry.boundsTree, _matrix$2, {
			...
			intersectsTriangles( triangleA, triangleB, ia, ib ) {
				...
				if ( triangleA.intersectsTriangle( triangleB, _edge$1, true ) && _edge$1.distance() > 1e-5 ) {
	...
	const _exTriangle = new threeMeshBvh.ExtendedTriangle();
	...
	class CullableTriangle extends three.Triangle {
	...
	function applyToGeometry( geometry, referenceGeometry, groups, attributeInfo ) {
		...
		for ( const key in rootAttrSet ) {
			...
			if ( ! attr || attr.array.length < requiredLength ) {
				...
				attr = new three.BufferAttribute( new type( requiredLength ), refAttr.itemSize, refAttr.normalized );
	...
	class PointsHelper extends three.InstancedMesh {
		...
		constructor( count = 1000, points = [] ) {
			...
			super( new three.SphereBufferGeometry( 0.025 ), new three.MeshBasicMaterial(), count );
			this.frustumCulled = false;
	...
	const _edgeCenter = new three.Vector3();
	const _edgeCenter2 = new three.Vector3();
